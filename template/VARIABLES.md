...
| `enable_todo` | bool | `false` | Includes the `pydantic-ai-todo` TODO planner capability: async PostgreSQL-backed (or in-memory fallback) task list that streams live checklist events to the client. Standalone opt-in; also serves as the planning backbone for `enable_deep_research` (which turns it on automatically). **PydanticAI only.** | Requires `use_pydantic_ai`; auto-enabled by `enable_deep_research` |
| `enable_subagents` | bool | `false` | Includes the `subagents-pydantic-ai` capability: spawns parallel researcher/analyst/writer subagents, with async polling and live status cards. Standalone opt-in; also used by `enable_deep_research` (which turns it on automatically). **PydanticAI only.** | Requires `use_pydantic_ai`; auto-enabled by `enable_deep_research` |
...
| `enable_memory` | bool | `false` | Persistent per-user agent memory (`pydantic-ai-harness`): the assistant keeps a markdown notebook (`MEMORY.md` + optional extra notes) per user and reads it back at the start of every conversation via bounded prompt injection (~2k tokens) — no RAG involved. The agent gets `write_memory` / `read_memory` / `delete_memory` / `search_memory` tools (each rendered as a chat card), users inspect, edit and delete every file on **Settings → Memory** (REST CRUD with optimistic-concurrency versions, stale save → 409), and memory steps are labeled in the demo replay's Agent's computer + run graph. Stored in self-managed `agent_memory*` PostgreSQL tables (created on first use, no Alembic migration). Activated at runtime with `ENABLE_MEMORY=true`. **PydanticAI only.** | Requires `use_pydantic_ai` + `--database postgresql` |
