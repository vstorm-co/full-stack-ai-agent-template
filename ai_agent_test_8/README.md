...
└── commands/             # Click CLI commands (auto-discovered by `ai_agent_test_8 cmd …`)
...
The generated project ships a Click CLI exposed as `ai_agent_test_8` (after `make install`):

```bash
ai_agent_test_8 server run --reload          # dev server
ai_agent_test_8 db upgrade                   # apply migrations
ai_agent_test_8 db migrate -m "message"      # create new migration
ai_agent_test_8 user create-admin            # interactive admin creation
ai_agent_test_8 rag-ingest <path> -c docs    # ingest local files
ai_agent_test_8 rag-search "query" -c docs   # semantic search
ai_agent_test_8 rag-collections              # list collections
```

Run `make help` for a categorized list, or `ai_agent_test_8 --help` for full CLI docs.
...
ai_agent_test_8 rag-ingest /path/to/docs/ --collection documents --recursive
# Pull from Google Drive (service-account auth)
ai_agent_test_8 rag-sync-gdrive --collection documents --folder-id <id>
# Pull from S3 / MinIO
ai_agent_test_8 rag-sync-s3 --collection documents --prefix docs/

# Semantic search
ai_agent_test_8 rag-search "your query" --collection documents
```
